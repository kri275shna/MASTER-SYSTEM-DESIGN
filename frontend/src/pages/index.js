...
export default function IndexPage() {
  ...
  useEffect(() => {
    ...
    const resolvedHost = (typeof process !== 'undefined' && process.env && process.env.NEXT_PUBLIC_API_URL) || 
                         (typeof window !== 'undefined' ? window.location.protocol + '//' + window.location.hostname + ':8000' : 'http://localhost:8000')
    setApiHost(resolvedHost)
  }, [])
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const mResp = await fetch(`${apiHost}/api/v1/stores/${storeId}/metrics`, { headers })
      if (mResp.ok) setMetrics(await mResp.json())
      
      // 2. Fetch Funnel
      const fResp = await fetch(`${apiHost}/api/v1/stores/${storeId}/funnel`, { headers })
      if (fResp.ok) setFunnel(await fResp.json())
      
      // 3. Fetch Heatmap
      const hResp = await fetch(`${apiHost}/api/v1/stores/${storeId}/heatmap`, { headers })
      if (hResp.ok) setHeatmap(await hResp.json())
      
      // 4. Fetch Anomalies
      const aResp = await fetch(`${apiHost}/api/v1/stores/${storeId}/anomalies`, { headers })
      if (aResp.ok) setAnomalies(await aResp.json())

      // 5. Fetch Cameras
      const cResp = await fetch(`${apiHost}/api/v1/stores/${storeId}/cameras`, { headers })
      if (cResp.ok) setCameras(await cResp.json())
      
    } catch (err) {
      console.error('Failed to load store data:', err)
    }
  }, [token, storeId, getHeaders, apiHost])

  useEffect(() => {
    if (token && mounted) {
      fetchDashboardData()
    }
  }, [token, mounted, fetchDashboardData])
  ...
  const handleResolveAnomaly = async (anomalyId, actionTaken) => {
    try {
      const response = await fetch(`${apiHost}/api/v1/stores/${storeId}/anomalies/${anomalyId}/resolve?action_taken=${encodeURIComponent(actionTaken)}`, {
        method: 'POST',
        headers: getHeaders()
      })
      if (response.ok) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch(`${apiHost}/api/v1/auth/login-json`, {
        method: 'POST',
  ...
  const triggerSimulation = async () => {
    ...
    try {
      ...
      // Send Entry Event
      await fetch(`${apiHost}/api/v1/events/ingest`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          store_id: storeId,
            ...
            <div className="flex items-center gap-6 overflow-x-auto text-xs whitespace-nowrap scrollbar-none w-full">
              {recentEvents.map((evt, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-slate-900/60 border border-slate-800/80 px-3 py-1 rounded-lg">
                  ...
                  <span className="font-mono text-slate-300">{evt.visitor_id || evt.payload?.visitor_id || 'unknown'}</span>
                </div>
              ))}
            </div>
          </div>
