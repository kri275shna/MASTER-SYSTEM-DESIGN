...
class FunnelService:
    def get_funnel_analytics(self, db: Session, store_id: str, start_time: datetime = None, end_time: datetime = None) -> Dict[str, Any]:
        """
        Computes visitor conversion funnel analytics:
        ENTRY -> ZONE VISIT -> BILLING QUEUE -> PURCHASE
        Also calculates drop-offs and average completion times.
        """
        ...
        # Get all non-staff visitor sessions
        ...
        # 2. Stage: ZONE_VISIT
        # Any session that has at least one ZONE_ENTER event
        ...
        # 3. Stage: BILLING_QUEUE
        # Any session that has at least one BILLING_QUEUE_JOIN event
        ...
        # 4. Stage: PURCHASE
        # Any session that is linked to a transaction or marked as converted
        ...
        # Load sessions details with events and transactions
        for s in sessions:
